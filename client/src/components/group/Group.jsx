  ...
  g1: {
    ...
    chat: [
      { id: "c1", sender: "You", text: "Booked the hotel!" },
      {
        id: "c2",
        sender: "Ravi",
        text: "Thanks! Will transfer my share soon.",
      },
    ],
  },
};
    ...
    <div className="flex h-screen bg-slate-900 text-slate-200 font-sans">
      ...
      <aside className="w-1/4 bg-slate-800 flex flex-col border-r border-slate-700">
        <div className="p-4 border-b border-slate-700 flex items-center gap-3 sticky top-0 bg-slate-800 z-10">
          <Link to="/groups" className="text-blue-400 hover:text-blue-300">
            &larr; Back
          </Link>
        </div>
        ...
        <form
          ...
          className="p-3 border-t border-slate-700 flex items-center gap-2 sticky bottom-0 bg-slate-800"
        >
          <input
            type="text"
            value={newMessage}
          ...
          <button
            type="submit"
            className="bg-blue-600 hover:bg-blue-500 rounded-full p-2 text-white transition-colors"
          >
            <svg
      ...
      <main className="w-1/2 flex flex-col">
        ...
        <div className="flex-grow p-6 overflow-y-auto">
          {group.expenses.map((exp) => (
            <div
              key={exp.id}
              className="bg-slate-800 p-4 rounded-lg flex justify-between items-center mb-3"
            >
      ...
      <aside className="w-1/4 bg-slate-800 border-l border-slate-700 p-6">
        ...
        <div className="text-2xl font-light mb-6">
          {group.balance > 0 && (
            <p>
              <strong className="text-slate-100">Group</strong> owes you{" "}
              <strong className="text-green-400">₹{group.balance}</strong>
            </p>
          )}
          {group.balance < 0 && (
            <p>
              You owe <strong className="text-slate-100">Group</strong>{" "}
          ...
          {group.balance === 0 && (
            <p className="text-slate-300">Group is all settled up 🎉</p>
          )}
        </div>
