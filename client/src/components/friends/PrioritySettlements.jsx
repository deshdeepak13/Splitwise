...
import UrgentCard from "../friends/UrgentCard"; // Assuming this already exists

const PrioritySettlements = ({ urgentDues, pendingRequestsCount }) => {
  return (
        ...
        <div className="mt-8">
          ...
          <div className="bg-gray-700/30 p-4 rounded-xl space-y-3 text-sm">
            <p className="flex items-center gap-3">
              ...
              <strong>{urgentDues.length}</strong> high-priority settlements to
              follow up on.
            </p>
            {urgentDues.length > 0 && urgentDues[0] && (
              <p className="flex items-center gap-3">
                ...
                pending amount is{" "}
                <strong>₹{urgentDues[0].balance.toLocaleString()}</strong> from{" "}
                <strong>{urgentDues[0].name}</strong>.
              </p>
            )}
            {pendingRequestsCount > 0 && (
              <p className="flex items-center gap-3">
                <FiClock className="text-purple-400" /> You have{" "}
                <strong>{pendingRequestsCount}</strong> pending friend requests.
              </p>
            )}
            {urgentDues.length === 0 && (
              <p className="flex items-center gap-3">
...
export default PrioritySettlements;